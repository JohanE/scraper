...
module Assignment
  class DataParser   
    ...
    def process_date(date_str)
      if ( date_str =~ /Idag/i )
        xx = Assignment::ParseHelper.get_date_as_string(Time.now) 
        yy = Assignment::ParseHelper.get_time_as_string(date_str)
        parsed_time = Time.parse("#{xx} #{yy}:00") 
      elsif ( date_str =~ /Ig.*r/i )
        xx = Assignment::ParseHelper.get_date_as_string(Time.now - (24 * 60 * 60)) 
        yy =  Assignment::ParseHelper.get_time_as_string(date_str)
        parsed_time = Time.parse("#{xx} #{yy}:00") 
